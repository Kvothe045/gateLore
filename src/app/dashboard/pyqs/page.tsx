...
export default function PYQArchive() {
  ...
  const handleExecuteSearch = async () => {
    ...
    try {
      // 1. Fetch a massive chunk of data (up to 200 questions) to guarantee a diverse pool
      ...
      if (res.ok) {
        ...
        // 2. The Fisher-Yates Shuffle (True Randomness)
  ...
  const handleCheckAnswer = async (q: PYQ) => {
    ...
    if (!isCorrect) {
      try {
        await fetch(`${process.env.NEXT_PUBLIC_CORTEX_API_URL}/api/pyqs/submit`, {
          method: "POST",
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 pb-32 animate-in fade-in duration-700">
      ...
      {/* THE COMBAT FEED (Centered & Focused) */}
      <main className="space-y-8">
        {!hasSearched ? null : displayedQuestions.length === 0 && !loading ? (
          <div className="p-16 text-center border border-zinc-800 border-dashed rounded-2xl bg-zinc-900/20">
          ...
          <div className="space-y-12">
            {/* HUD Header */}
            <div className="flex items-center justify-between text-xs font-mono font-bold uppercase tracking-widest text-zinc-500 px-2">
              <span>Displaying {displayedQuestions.length} of {questionPool.length} Scrambled Entities</span>
              <span className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" /> Live Verification Active</span>
            </div>
              ...
              return (
                <div key={q.id} className={cn(
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 p-5 md:p-6 bg-zinc-900/30 border-b border-zinc-800/50">
                    ...
                    <div className="px-3 py-1 bg-zinc-900 border border-zinc-700 text-zinc-400 rounded-md text-xs font-black tracking-widest uppercase shadow-inner">
                      {q.question_type}
                    </div>
                  </div>

                  {/* Q Body (Using SmartDisplay) */}
                            ...
                            return (
                              <button 
                                ...
                                className={cn(
                                  ...
                                  isWrongSelection ? "bg-rose-950/10 border-rose-500/30 opacity-60" : "bg-zinc-950/30 border-zinc-800/30 opacity-30"
                                )}
                              >
                                <div className={cn(
                                  ...
                                  {isCorrectOption ? <Check className="w-5 h-5" /> : opt.label}
                                </div>
                                <div className="ml-5">
                    ...
                    {!isLocked ? (
                      ...
                      <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                          ...
                          {fb.status === "wrong" && (
                            <span className="flex items-center gap-2 text-xs font-bold tracking-widest uppercase text-amber-500/80 bg-zinc-950 border border-zinc-800/80 px-4 py-2.5 rounded-lg">
                              <AlertTriangle className="w-4 h-4" /> Logged to Hospital
                            </span>
                          )}
                        </div>
                        ...
                        {q.solution_url && (
                          <a 
                            ...
                            className="flex items-center gap-2 text-xs font-bold tracking-widest uppercase text-cyan-400 hover:text-cyan-300 bg-cyan-950/20 hover:bg-cyan-950/40 border border-cyan-500/20 px-5 py-2.5 rounded-lg transition-all"
                          >
            ...
            {displayCount < questionPool.length && (
              <div className="pt-6 pb-12 flex justify-center">
                <button 
                  ...
                  className="bg-zinc-900 border-2 border-zinc-800 hover:border-cyan-500/40 hover:bg-zinc-900/80 text-zinc-400 hover:text-cyan-400 px-10 py-4 rounded-xl font-bold tracking-widest uppercase text-xs transition-all shadow-lg"
                >
                  Load Next Sequence
                </button>
              </div>
