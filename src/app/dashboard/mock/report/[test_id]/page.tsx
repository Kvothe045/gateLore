...
export default function PostExamReport({ params }: { params: Promise<{ test_id: string }> }) {
  ...
  if (!report) return <div className="p-10 text-center text-rose-500 font-mono border border-rose-500/20 bg-rose-950/10 rounded-xl mt-10">Report Data Corrupted or Not Found.</div>;

  // Derive active data for the drill-down view (Filtering out empty 0-mark subjects)
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-32 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard title="System Accuracy" value={`${report.accuracy}%`} icon={Crosshair} color={report.accuracy > 70 ? "text-emerald-400" : "text-rose-400"} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-6">
        ...
        <aside className="lg:col-span-4 space-y-6">
          ...
          {/* Subject Grid (Now strictly filtered) */}
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className={cn(
                      ...
                      subj.accuracy > 70 ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"
                    )}>
                      {subj.accuracy}%
                    </span>
                  </div>
        ...
        <main className="lg:col-span-8 space-y-8">
          ...
          {/* Topic Sub-View (Dynamic based on selected Subject) */}
          {currentSubjectData && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-zinc-800/60 mb-6">
                ...
                <Link 
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2.5 bg-purple-600/10 hover:bg-purple-600/20 border border-purple-500/30 text-purple-400 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all"
                >
                  <RotateCcw className="w-3.5 h-3.5" /> Retry {activeTopic === "ALL" ? "Subject" : "Topic"}
                </Link>
              </div>

              {/* Topic Chips (Filtered out empty topics just in case) */}
              <div className="flex flex-wrap gap-2 mb-8">
                <button
                  onClick={() => setActiveTopic("ALL")}
                  className={cn(
                    "px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest border transition-all",
                    activeTopic === "ALL" ? "bg-cyan-600 text-white border-cyan-500" : "bg-zinc-900/50 border-zinc-800 text-zinc-500 hover:border-zinc-700"
                  )}
                >
                  All Topics
                </button>
                {currentSubjectData.topics.filter((t: any) => t.attempted > 0 || t.wrong > 0 || t.correct > 0).map((t: any) => (
                  <button
                    ...
                    className={cn(
                      ...
                      activeTopic === t.topic ? "bg-cyan-600 text-white border-cyan-500" : "bg-zinc-900/50 border-zinc-800 text-zinc-500 hover:border-zinc-700"
                    )}
                  >
                    {t.topic}
                    <span className={cn(
              ...
              <div className="space-y-8">
                {filteredQuestions.length === 0 ? (
                  ...
                  filteredQuestions.map((resp: any, idx: number) => {
                    const userKeys = resp.user_keys || [];
                    const correctKeys = resp.correct_keys || [];
                    ...
                    return (
                      <div key={resp.question_id} className={cn(
                        ...
                        resp.is_correct ? "border-emerald-500/20 bg-emerald-950/5" : "border-rose-500/20 bg-rose-950/5"
                      )}>
                        
                        {/* Q Header */}
                        <div className={cn(
                          "p-5 border-b flex flex-wrap justify-between items-center gap-4",
                          resp.is_correct ? "bg-emerald-500/10 border-emerald-500/20" : "bg-rose-500/10 border-rose-500/20"
                        )}>
                          <div className="flex items-center gap-4">
                            <div className={cn(
                              "w-10 h-10 rounded-lg flex items-center justify-center border shadow-inner",
                              resp.is_correct ? "bg-emerald-500 border-emerald-400 text-zinc-950" : "bg-rose-500 border-rose-400 text-white"
                            )}>
                              {resp.is_correct ? <CheckCircle2 className="w-6 h-6" /> : <XCircle className="w-6 h-6" />}
                            </div>
                            <div>
                              <h4 className="font-black text-zinc-100 text-sm flex items-center gap-2">
                                Question {idx + 1}
                                {isBlank && <span className="px-2 py-0.5 rounded text-[9px] font-black uppercase bg-zinc-950 border border-zinc-800 text-amber-500">Unanswered</span>}
                              </h4>
                            ...
                            <div className="grid md:grid-cols-2 gap-6 bg-zinc-900/50 p-6 rounded-xl border border-zinc-800 mb-6">
                              <div className="space-y-2">
                                ...
                                <div className={cn(
                                  ...
                                  resp.is_correct ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "bg-rose-500/10 border-rose-500/30 text-rose-400"
                                )}>
                                  {isBlank ? "EMPTY" : userKeys[0]}
                                </div>
                              </div>
                              <div className="space-y-2">
